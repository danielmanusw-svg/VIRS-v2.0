...
const styles = StyleSheet.create({
  ...
  reconDiff: {
    fontSize: 9,
    fontFamily: "Helvetica-Bold",
    color: "#DC2626", // Red for discrepancies
  },
});
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Reconciliation Summary Highlighting Gaps */}
        <View style={styles.reconBox}>
          <Text style={styles.reconTitle}>SYSTEM RECONCILIATION SUMMARY</Text>
          <View style={styles.reconRow}>
            <Text style={styles.reconLabel}>Total Orders Processed:</Text>
            <Text style={styles.reconValue}>{lines.length > 0 ? new Set(lines.map(l => l.order_number)).size : 0}</Text>
          </View>
          <View style={styles.reconRow}>
            <Text style={styles.reconLabel}>Discrepancy Status:</Text>
            <Text style={missingOrderNumbers.length > 0 ? styles.reconDiff : styles.reconValue}>
              {missingOrderNumbers.length > 0 ? "ACTION REQUIRED: ASYNC DATA" : "MATCHED"}
            </Text>
          </View>
        </View>
        ...
        <Text style={styles.systemStamp}>
          VIRS AUDIT ID: {Math.random().toString(36).substring(2, 10).toUpperCase()} MATCH-SECURED
        </Text>
      </Page>
    </Document>
  );
